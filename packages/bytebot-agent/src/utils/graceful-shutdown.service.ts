...
export class GracefulShutdownService implements OnApplicationShutdown {
  ...
  constructor(private readonly logger: BytebotLogger) {
    ...
    process.on('uncaughtException', (error) => {
      this.logger.error('Uncaught exception, shutting down gracefully', error.stack, 'GracefulShutdown');
      this.handleShutdown('uncaughtException');
    });
    process.on('unhandledRejection', (reason) => {
      this.logger.error('Unhandled rejection, shutting down gracefully', String(reason), 'GracefulShutdown');
  ...
  registerShutdownHandler(handler: ShutdownHandler): void {
    ...
    this.logger.debug(`Registered shutdown handler: ${handler.name} (priority: ${handler.priority})`, 'GracefulShutdown');
  }
  ...
  private async handleShutdown(signal: string): Promise<void> {
    if (this.isShuttingDown) {
      this.logger.warn('Shutdown already in progress, ignoring signal', 'GracefulShutdown');
  ...
  private async executeShutdown(): Promise<void> {
    ...
    try {
      for (const handler of this.handlers) {
        ...
        try {
          this.logger.info(`Executing shutdown handler: ${handler.name}`, 'GracefulShutdown');
          ...
          this.logger.info(`Shutdown handler completed: ${handler.name} (${duration}ms)`, 'GracefulShutdown');
          completedHandlers++;
      ...
      this.logger.error('Error during graceful shutdown', error.stack, 'GracefulShutdown');
    } finally {
      // Force exit after a reasonable timeout
      setTimeout(() => {
        this.logger.warn('Force exiting after shutdown timeout', 'GracefulShutdown');
        process.exit(1);
  ...
  private createTimeoutPromise(timeout: number, handlerName: string): Promise<never> {
    return new Promise((_, reject) => {
      setTimeout(() => {
        reject(new Error(`Shutdown handler timeout: ${handlerName} (${timeout}ms)`));
      }, timeout);
    });
  }
