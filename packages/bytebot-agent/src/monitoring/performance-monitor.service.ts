...
export class PerformanceMonitorService {
  ...
  async generatePerformanceReport(
    ...
    const averageTokenUsage =
      recentMetrics.reduce((sum, m) => sum + m.tokenUsage, 0) /
      recentMetrics.length;
    const averageIterations =
      recentMetrics.reduce((sum, m) => sum + m.iterationCount, 0) /
      recentMetrics.length;
  ...
  private checkForPerformanceIssues(metrics: PerformanceMetrics): void {
    ...
    if (metrics.memoryUsage > 1024 * 1024 * 1024) {
      // 1GB
      this.logger.warn(
        `High memory usage detected for task ${metrics.taskId}: ${Math.round(metrics.memoryUsage / 1024 / 1024)}MB`,
      );
    }
    ...
    if (metrics.cpuUsage > 80) {
      this.logger.warn(
        `High CPU usage detected for task ${metrics.taskId}: ${metrics.cpuUsage}%`,
      );
    }
    ...
    if (metrics.duration > 60000) {
      ...
      this.logger.warn(
        `Long iteration duration for task ${metrics.taskId}: ${Math.round(metrics.duration / 1000)}s`,
      );
    }
    ...
    if (metrics.tokenUsage > 100000) {
      this.logger.warn(
        `High token usage for task ${metrics.taskId}: ${metrics.tokenUsage} tokens`,
      );
    }
  }
  ...
  private identifyBottlenecks(
    ...
    const highMemoryTasks = metrics.filter(
      (m) => m.memoryUsage > 512 * 1024 * 1024,
    ); // 512MB
    if (highMemoryTasks.length > metrics.length * 0.2) {
      // 20% of tasks
      bottlenecks.push({
        type: 'memory',
        severity: 'high',
    ...
    const avgIterations =
      metrics.reduce((sum, m) => sum + m.iterationCount, 0) / metrics.length;
    if (avgIterations > 20) {
      bottlenecks.push({
  ...
  private generateRecommendations(
    ...
    if (avgDuration > 30000) {
      // 30 seconds
      recommendations.push(
        'Consider implementing parallel processing for long-running tasks',
      );
    }

    const avgTokens =
      metrics.reduce((sum, m) => sum + m.tokenUsage, 0) / metrics.length;
    if (avgTokens > 30000) {
      recommendations.push(
        'Implement more aggressive context management to reduce token usage',
      );
    }
    ...
    if (avgMemory > 256 * 1024 * 1024) {
      // 256MB
      recommendations.push(
        'Implement memory cleanup routines to prevent memory leaks',
      );
    }
