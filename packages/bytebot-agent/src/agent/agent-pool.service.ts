...
export class AgentPoolService {
  ...
  private distributionStrategy: TaskDistributionStrategy = { type: 'intelligent' };
  ...
  unregisterAgent(agentId: string): boolean {
    ...
    if (agent.currentTask) {
      this.logger.warn(`Unregistering agent ${agentId} that is processing task ${agent.currentTask}`);
  ...
  async distributeTask(task: Task): Promise<string | null> {
    ...
    if (availableAgents.length === 0) {
      this.logger.warn(`No available agents for task ${task.id}, adding to queue`);
    ...
    try {
      ...
      this.logger.error(`Failed to start task ${task.id} on agent ${selectedAgent.id}: ${error.message}`);
  ...
  async distributeParallelTasks(tasks: Task[]): Promise<Map<string, string[]>> {
    ...
    this.logger.log(`Distributed ${sortedTasks.length} tasks across ${assignments.size} agents`);
    return assignments;
  }
  ...
  async handleAgentFailure(agentId: string, taskId: string): Promise<void> {
    this.logger.warn(`Handling failure of agent ${agentId} processing task ${taskId}`);
    ...
    if (task && task.status === TaskStatus.RUNNING) {
      ...
      if (reassignedAgentId) {
        this.logger.log(`Reassigned task ${taskId} to agent ${reassignedAgentId}`);
      } else {
        this.logger.error(`Failed to reassign task ${taskId} - no available agents`);
      }
    }
  }
   ...
   */
  updateAgentPerformance(agentId: string, taskId: string, duration: number, success: boolean): void {
    const agent = this.agents.get(agentId);
    if (!agent) return;
    ...
    agent.performance.averageTaskDuration = 
      (agent.performance.averageTaskDuration * agent.performance.tasksCompleted + duration) / completedTasks;
    
    agent.performance.successRate = 
      (agent.performance.successRate * agent.performance.tasksCompleted + (success ? 1 : 0)) / completedTasks;
    ...
    this.logger.debug(`Updated performance for agent ${agentId}: ${completedTasks} tasks, ${Math.round(agent.performance.successRate * 100)}% success rate`);
   ...
   */
  updateAgentResources(agentId: string, cpuUsage: number, memoryUsage: number): void {
    const agent = this.agents.get(agentId);
    if (!agent) return;
  ...
  getPoolStats() {
    ...
    return {
      ...
      totalTasksCompleted,
      averageSuccessRate,
      averageLoadFactor: agents.length > 0 
        ? agents.reduce((sum, a) => sum + a.resourceUsage.loadFactor, 0) / agents.length 
        : 0,
      topPerformers: agents
        .sort((a, b) => (b.performance.successRate * b.performance.tasksCompleted) - (a.performance.successRate * a.performance.tasksCompleted))
        .slice(0, 3)
        .map(a => ({
          id: a.id,
  ...
  private getAvailableAgents(): AgentInstance[] {
    return Array.from(this.agents.values()).filter(agent => 
      agent.isAvailable && agent.resourceUsage.loadFactor < 0.8
    );
  }

  private selectBestAgent(availableAgents: AgentInstance[], task: Task): AgentInstance | null {
    switch (this.distributionStrategy.type) {
      case 'round_robin':
        return availableAgents[0]; // Simple first available

      case 'least_loaded':
        return availableAgents.sort((a, b) => a.resourceUsage.loadFactor - b.resourceUsage.loadFactor)[0];

      case 'priority_based':
        if (task.priority === TaskPriority.HIGH) {
          // Assign high priority tasks to best performing agents
          return availableAgents.sort((a, b) => b.performance.successRate - a.performance.successRate)[0];
        }
        return availableAgents.sort((a, b) => a.resourceUsage.loadFactor - b.resourceUsage.loadFactor)[0];
  ...
  private selectIntelligentAgent(availableAgents: AgentInstance[], task: Task): AgentInstance | null {
    ...
    const scoredAgents = availableAgents.map(agent => {
      ...
      const experienceFactor = Math.min(agent.performance.tasksCompleted / 100, 1);
      score += experienceFactor * 20;

      // Priority bonus (0-10 points)
      if (task.priority === TaskPriority.HIGH && agent.performance.successRate > 0.8) {
        score += 10;
      }
